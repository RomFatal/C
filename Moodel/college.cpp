...
void College::addDepartment(string depName) {
	departmentsList.push_back(Department(depName));
}
...
bool College::updateCourseDetails(const int courseId, const int numOfCred, const string courseName, const string depName) {
	...
	while (cur != 0) {
		if (courseId == cur->Val->getCourseId()) {
			...
			for (i = 0, size = departmentsList.size(); i < size; ++i) {//find the department
				if (departmentsList[i].getDepartmentName() == depName) {
					departmentsList[i].addCourse(cur->Val);//chain the course to the head of the depattment's courses list
					return true;
				}
			}
		}
...
void College::addStudent(const string fName, const string lName, const int ID, const int rYear, const string studAd, const string dep, const int sumOfCred) {
	size_t i, size;
	bool addedTodep = false, addedTocyc = false;
	for (i = 0, size = departmentsList.size(); i < size; ++i) {//add to department
		if (departmentsList[i].getDepartmentName() == dep) {
			addedTodep = true;
			break;
		}
	}
	if (!addedTodep)
		return;
	RCPtr<Student>&nStudent = departmentsList[i].addStudent(fName, lName, ID, rYear, studAd, dep, sumOfCred);
...
void College::signUpCourse(const int id, const int courseId) {
	string courseDepartment = findCourseDepartment(courseId);
	if (courseDepartment == "")
		return;
	bool departmentFound = false;
	size_t i, size;
	for (i = 0, size = departmentsList.size(); i < size; ++i) {
		if (departmentsList[i].getDepartmentName() == courseDepartment) {
	...
	if (!departmentFound)
		return;
	try {
		departmentsList[i].signUpCourse(id, courseId);
	}
	catch (exception()) {
		return;
	}
...
void College::graduateStudent(const int Id, const int courseId) {
	nodeType<Course>* curCourse = getCourseById(courseId);
	if (!curCourse)
		return;
	curCourse->Val->graduateStudent(Id, true);
}
void College::removeStudent(const int Id, const int courseId) {
	nodeType<Course>* curCourse = getCourseById(courseId);
	if (!curCourse)
		return;
	curCourse->Val->graduateStudent(Id, false);
}
void College::deleteStudent(const int Id, const int cycle) {
	...
	for (j = 0, depatrmentsSize = departmentsList.size(); j < depatrmentsSize; ++j) {
		if (departmentsList[j].isExist(Id) == true) {
			departmentsList[j].deleteStudent(Id);
			break;
		}
	}
}
